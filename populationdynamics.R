...
# NUEVOS MODELOS
{
  ...
  f5.p.gg <- ggplot(f5.p.df, aes(x=offspring.h1)) + 
   ...
   # ggsave("/Users/gdh/Documents/MEGA/Tesis/Gráficas/fc5-gg.png", f5.p.gg, device = "png", width = 10, height = 7, units = "in", dpi = 180*2)
 	...
 	new.stb.n <- data.frame(Age = und.ages[order(und.ages)], SNB = rep(NA, length(und.ages)))
 	for (i in 1:nrow(stb.n)) {
 		if (stb.n$Age[i] == new.stb.n$Age[which(new.stb.n$Age == stb.n$Age[i])]) {
 			new.stb.n$SNB[which(new.stb.n$Age == stb.n$Age[i])] <- stb.n$SNB[i]
 		}
 	}
 	new.stb.n$SNB[which(is.na(new.stb.n$SNB) == TRUE)] <- 0
 	stb.n <- new.stb.n
 	stb.n <- transform(stb.n, Age = as.numeric(as.character(Age)))
 	n.ages <- stb.n$Age
 	f4.a <- rep(NA, length(n.ages))
 	for (a in n.ages) {
   		Mim.a <- subset(Survival, Age == a)
   		n.str.a <- as.data.frame(cbind(x.pred, hist(log(as.numeric(as.character(Mim.a$h1))), breaks = e.pred, plot = FALSE)$counts))
   		names(n.str.a) <- c("ln.h1", "n")
   		n.vec.a <- n.str.a$ln.h1
   		#pred.s.1 <- predict(glmm.s, newdata = data.frame(ln.h1 = n.vec.1, Age = rep(1, length(n.vec.1))), type = "link", backtransform = TRUE, re.form = NA)
   		#pred.g.1 <- predict(lmm.g, newdata = data.frame(ln.h1 = n.vec.1, Age = rep(1, length(n.vec.1))), type = "response", re.form = NA)
   		...
   		#pred.p.1 <- (pred.s.1*pred.g.1) ### ?
   		pred.f.a <- pred.f1.a*pred.f2.a*pred.f3.a
   		new.n.a <- sum(pred.f.a) # número de semillas producidos por el proceso reproductivo en el año anterior
   		stb.n.a <- stb.n$SNB[which(stb.n$Age == a)] # número de reclutas observados
   		f4.a[which(stb.n$Age == a)] <- stb.n.a/new.n.a
 		}
 	f4.a[which(f4.a == 0)] <- 0.00000001 # cambiar los 0s para que corra la regresion beta
 	#GAMs #establishment probability over time
 	x <- n.ages
 	#gam.f4 <- gam(f4.a~s(x, k = 3), family = gaussian(log))
    #glm.f4 <- betareg(f4.a~x)
