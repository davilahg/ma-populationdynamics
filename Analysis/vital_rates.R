...
# read data
{
    ...
    lhmean <- mean(c(data$ln.h1, data$ln.h2), na.rm = TRUE) ## tienen que estar también los reclutas
...
{
 ## s
 glmm.s <- glmer(sup~ln.h1_scl*Age_scl+(1+Age_scl*ln.h1_scl| Census)+(1+ln.h1_scl|PlotCode), survival, binomial) # survival: s
 ## g
 lmm.g <- lmer(ln.h2_scl~ln.h1_scl*Age_scl+(1+ln.h1_scl| Census)+(1+Age_scl|PlotCode), growth) # growth: g
 ## f1
...
# vital rates plots
{
 # definitions
 {
    bin.size <- 0.1
    n.sim <- 1000
    h1.pred <- seq(min(survival$h1_scl, na.rm = TRUE), ceiling(max(survival$h1_scl, na.rm = TRUE)), length.out = m)
    h2.pred <- seq(min(survival$h2_scl, na.rm = TRUE), ceiling(max(survival$h2_scl, na.rm = TRUE)), length.out = m+1)
    mids.h2 <- hist(survival$h2_scl, breaks = h2.pred, plot = FALSE)$mids
    ...
    h1.pred.1 <- seq(min(survival$h1_scl, na.rm = TRUE), ceiling(max(survival$h1_scl, na.rm = TRUE)), length.out = m+1)
    mids <- hist(survival$h1_scl, breaks = h1.pred.1, plot = FALSE)$mids
    ln.mids <- log(mids)
 }
 # Survival
 {
